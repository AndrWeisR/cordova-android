...
function updateProjectAccordingTo (platformConfig, locations) {
    ...
    // RTWSA workaround: Change all \ in newDestFile to /
    //const newDestFile = path.join(locations.root, 'app', 'src', 'main', 'java', androidPkgName.replace(/\./g, '/'), path.basename(destFile)).replace(/\\/g, '/');
    const newDestFile = path.join(locations.root, 'app', 'src', 'main', 'java', androidPkgName.replace(/\./g, '/'), path.basename(destFile));
    events.emit('verbose', `destFile = ${destFile} newDestFile = ${newDestFile}`);
